function [k,lambda_l,lambda_d,lambda_t,num_iter]=alg_BGcmf_parest(Y,Sd,St,cv_setting,nr_fold,left_out,use_WKNKN,K,eta,use_W_matrix)
...
%K, lambda_l, lambda_d & lambda_t.
    ...
    range_lambda_dt  = [   2^-5   2^-4   2^-3   2^-2   2^-1   2^4   2^5];
            ...
            for lambda_d=range_lambda_dt
                for lambda_t=range_lambda_dt
                    ...
                    for i=1:nr_fold
                        ...
                        if ~use_W_matrix                                                                    %
                            [A,B] = alg_BGcmf_predict(y2,A,B,Sd,St,lambda_l,lambda_d,lambda_t,num_iter);      %
                        else                                                                                % PREDICT
                            [A,B] = alg_BGcmf_predict(y2,A,B,Sd,St,lambda_l,lambda_d,lambda_t,num_iter,W);    %
                        end                                                                                 %
                        y3 = A*B';                                                                          %
